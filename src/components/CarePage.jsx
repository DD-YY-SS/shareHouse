...
export default function CarePage({ roomId, matchId, auth, back }) {
  ...
  return <section className="page-pad inner-page care-page">
    ...
    <section className="presence-card"><div><span className="section-kicker">ROOMMATE STATUS</span><strong>{ownOption.icon} {ownOption.label}</strong><small>룸메이트: {roommateOption.icon} {roommateOption.label}</small></div><button type="button" onClick={() => setPresenceOpen(true)}>상태 변경</button></section>
