import { React, useEffect, useState, API, BarChart3, Check, Clock3, UsersRound } from '../shared.js';
import QRCode from 'qrcode';
import './live-demo.css';

const initialState = { target: 30, participants: 0, surveyCompleted: 0, matched: 0, recentParticipants: [], bestCouple: null, bestCoupleRevealed: false, lastActivityAt: null, matchRevealed: false, chatEnabled: false };
...
export default function LiveDemo() {
  ...
  useEffect(() => {
    const qrTrigger = document.querySelector('.live-qr-placeholder');
    if (!qrTrigger) return undefined;
    qrTrigger.setAttribute('role', 'button');
    qrTrigger.setAttribute('tabindex', '0');
    qrTrigger.setAttribute('aria-label', 'QR 코드 크게 보기');
  ...
  const load = async () => {
    ...
    } catch { /* 발표 화면은 API 재연결 중에도 유지합니다. */ }
  };

  useEffect(() => { let cancelled = false; const run = async () => { if (!cancelled) await load(); }; run(); const timer = setInterval(run, 2000); return () => { cancelled = true; clearInterval(timer); }; }, []);

  const control = async (action) => {
    setControlBusy(action);
    setControlError('');
    try {
      const path = action === 'reveal' ? 'matching/reveal' : action === 'chat' ? 'chat/activate' : action === 'bestCouple' ? 'best-couple/reveal' : 'reset';
      const response = await fetch(`${API}/api/v1/live-demo/${path}`, { method: 'POST' });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.message || data.error || '라이브 상태를 변경하지 못했어요.');
      await load();
    } catch (error) {
      setControlError(error.message || '라이브 상태를 변경하지 못했어요.');
  ...
  return <main className="live-presenter-shell">
    ...
    {state.chatEnabled && <button className="live-best-control" disabled={Boolean(controlBusy) || state.bestCoupleRevealed} onClick={() => control('bestCouple')}>{controlBusy === 'bestCouple' ? '베스트 커플 확인 중...' : state.bestCoupleRevealed ? '오늘의 베스트 커플 공개됨' : '베스트 커플 확인하기'}</button>}
    ...
    <section className="live-hero"><div><span className="section-kicker">OFFLINE LIVE DEMO</span><h1>오늘 우리 공간에서<br /><em>가장 잘 맞는 룸메이트</em>를 찾아요.</h1><p>참가자가 접속하고 설문을 완료하는 과정을<br />발표자 화면에서 실시간으로 확인해 보세요.</p></div><div className="live-qr-placeholder"><span>QR</span><small>참가자 접속</small></div></section>

    <section className="live-control-bar"><div><strong>발표자 컨트롤</strong><small>버튼을 누르면 모든 참가자 화면에 즉시 반영됩니다.</small></div><div><button className={state.matchRevealed ? 'control-done' : ''} disabled={Boolean(controlBusy) || state.matchRevealed} onClick={() => control('reveal')}>{controlBusy === 'reveal' ? '생성 중...' : state.matchRevealed ? '매칭 결과 공개됨' : '매칭 결과 생성'}</button><button className={state.chatEnabled ? 'control-done' : ''} disabled={Boolean(controlBusy) || !state.matchRevealed || state.chatEnabled} onClick={() => control('chat')}>{controlBusy === 'chat' ? '활성화 중...' : state.chatEnabled ? '채팅방 활성화됨' : '채팅방 활성화'}</button><button className="control-reset" disabled={Boolean(controlBusy)} onClick={() => control('reset')}>초기화</button></div></section>

    <section className="live-stage-grid"><div className="live-live-card"><div className="live-card-title"><div><UsersRound size={20} /><strong>실시간 참여 현황</strong></div><small>2초마다 갱신</small></div><div className="live-big-count"><strong>{state.participants}</strong><span>/ {state.target}명</span></div><div className="live-progress"><i style={{ width: `${participantProgress}%` }} /></div><p>현재 접속 중인 참가자 수</p><div className="live-stats-row"><div><strong>{state.surveyCompleted}</strong><span>설문 완료</span><small>{surveyProgress}%</small></div><div><strong>{state.matched}</strong><span>매칭 분석</span><small>{matchProgress}%</small></div><div><strong>{updatedAt ? updatedAt.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' }) : '--:--'}</strong><span>마지막 갱신</span><small>LIVE</small></div></div></div><div className="live-funnel-card"><div className="live-card-title"><div><BarChart3 size={20} /><strong>시연 진행 단계</strong></div><small>순서대로 진행</small></div><div className="live-step done"><span>1</span><div><strong>접속 및 프로필</strong><small>참가자 기본 정보 입력</small></div><Check size={17} /></div><div className={`live-step ${state.surveyCompleted ? 'done' : 'active'}`}><span>2</span><div><strong>생활 패턴 설문</strong><small>{state.surveyCompleted}명 완료</small></div>{state.surveyCompleted ? <Check size={17} /> : <i />}</div><div className={`live-step ${state.matchRevealed ? 'done' : 'active'}`}><span>3</span><div><strong>룰베이스 매칭</strong><small>{state.matchRevealed ? '1순위 결과 공개' : '발표자의 공개를 기다리는 중'}</small></div>{state.matchRevealed ? <Check size={17} /> : <i />}</div><div className={`live-step ${state.chatEnabled ? 'done' : ''}`}><span>4</span><div><strong>30분 안심 채팅</strong><small>{state.chatEnabled ? '참가자 입장 가능' : '발표자의 활성화를 기다리는 중'}</small></div>{state.chatEnabled ? <Check size={17} /> : <Clock3 size={17} />}</div></div></section>

    {state.matchRevealed && <section className="live-best-couple"><div className="live-card-title"><div><span className="best-couple-mark">♥</span><strong>오늘의 베스트 커플</strong></div><small>전체 참가자 중 최고 궁합</small></div>{state.bestCouple ? <><div className="best-couple-people">{state.bestCouple.participants.map((person) => <div className="best-couple-person" key={person.id}>{person.profilePhotoData ? <img src={person.profilePhotoData} alt="" /> : <span className="live-avatar">{String(person.pseudonym || '?').slice(0, 1)}</span>}<strong>{person.pseudonym || '익명 참가자'}</strong><small>{person.age ? `${person.age}세` : '나이 비공개'}{person.mbti ? ` · ${person.mbti}` : ''}</small></div>)}</div><div className="best-couple-score"><strong>{state.bestCouple.score}%</strong><span>생활 궁합</span></div></> : <p className="live-empty">설문을 완료한 참가자가 두 명 이상이면 베스트 커플이 표시됩니다.</p>}</section>}
    ...
    {qrOpen && <div className="qr-modal-backdrop" role="dialog" aria-modal="true" aria-label="참가자 접속 QR 코드" onClick={() => setQrOpen(false)}><div className="qr-modal" onClick={(event) => event.stopPropagation()}><button type="button" className="qr-modal-close" onClick={() => setQrOpen(false)} aria-label="QR 코드 닫기">×</button><span className="section-kicker">PARTICIPANT ACCESS</span><h2>참가자 접속 QR</h2><p>청중들이 휴대폰 카메라로 스캔하면 참가자 화면으로 이동합니다.</p>{qrDataUrl ? <img className="qr-modal-code" src={qrDataUrl} alt="참가자 접속 QR 코드" /> : <div className="qr-loading">QR 생성 중...</div>}<strong>{participantUrl}</strong></div></div>}
  </main>;
}
