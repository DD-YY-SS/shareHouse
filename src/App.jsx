...
function AuthenticatedApp(){
 ...
 useEffect(()=>{if(!auth?.accessToken)return;let cancelled=false;const key=`cm-care-start-${auth.user.id}`;const restore=async()=>{try{const data=await responseJson(await fetch(API+'/api/v1/matches/candidates',{headers:{Authorization:'Bearer '+auth.accessToken}}));if(cancelled)return;const active=data.activeMatch&&data.candidates?.find(c=>data.activeMatch.memberIds.includes(c.candidateId));if(!active&&!data.activeMatch){sessionStorage.removeItem(key);if(match?.matchId)setMatch(null);return}const source=active||{candidateId:data.activeMatch.memberIds.find(id=>id!==auth.user.id),pseudonym:'Anonymous tenant',score:data.activeMatch.compatibility,breakdown:data.activeMatch.breakdown,bestReasons:data.activeMatch.bestReasons,watchouts:data.activeMatch.watchouts,status:data.activeMatch.status,accepted:true,matchId:data.activeMatch.id,preferences:data.activeMatch.preferences};const restored={...source,matchId:source.matchId||data.activeMatch.id,status:source.status||data.activeMatch.status,totalCandidates:data.totalCandidates||data.candidates?.length||0};setMatch(restored);if(restored.status==='confirmed'){let startAt=sessionStorage.getItem(key);try{const payment=await responseJson(await fetch(`${API}/api/v1/matches/${restored.matchId}/payment-status`,{headers:{Authorization:'Bearer '+auth.accessToken}}));if(payment.allTenantsPaid)startAt=startAt||payment.payment?.paidAt||new Date().toISOString()}catch{}sessionStorage.setItem(key,startAt||new Date().toISOString())}else sessionStorage.removeItem(key)}catch{}};restore();return()=>{cancelled=true}},[auth?.accessToken]);
 ...
 const logout=()=>{if(auth?.user?.id)sessionStorage.removeItem('cm-match-'+auth.user.id);sessionStorage.removeItem('cm-auth');setAuth(null);setMatch(null);setChat(null);setPage('entry')};
 ...
 const openMatching=async()=>{const careKey=auth?.user?.id?`cm-care-start-${auth.user.id}`:'';if((careKey&&sessionStorage.getItem(careKey))||match?.status==='confirmed'){go('care');return}if(match){go('match');return}if(!auth?.accessToken){go('survey');return}setLoading(true);try{const data=await responseJson(await fetch(API+'/api/v1/matches/candidates',{headers:{Authorization:'Bearer '+auth.accessToken}}));const active=data.activeMatch?{candidateId:data.activeMatch.memberIds.find(id=>id!==auth.user.id),pseudonym:'Anonymous tenant',score:data.activeMatch.compatibility,breakdown:data.activeMatch.breakdown,bestReasons:data.activeMatch.bestReasons,watchouts:data.activeMatch.watchouts,status:data.activeMatch.status,accepted:true,matchId:data.activeMatch.id,preferences:data.activeMatch.preferences,totalCandidates:data.totalCandidates}:null;const current=data.candidates?.find(c=>data.activeMatch?.memberIds?.includes(c.candidateId));if((current||active)?.status==='confirmed'){sessionStorage.setItem(careKey,sessionStorage.getItem(careKey)||new Date().toISOString());setMatch(current||active);go('care')}else if(current||active){setMatch(current||active);go('match')}else go('survey')}catch{go('survey')}finally{setLoading(false)}};
 ...
 return <div className="app-shell"><header className="topbar"><button className="brand" onClick={()=>go('entry')}><img className="app-logo top-logo" src="/checkmate-logo.svg" alt="CheckMate logo"/><span>CheckMate</span></button></header><main>
  {page==='entry'&&<Entry context={context} consent={consent} setConsent={setConsent} next={()=>go('survey')} profileComplete={profileComplete} openMatching={openMatching} resident={match?.status==='confirmed'}/>}
